...
%global npm_bare      csharp-config
%global sitelib       %{_prefix}/lib/node_modules

Name:           nodejs-sebastienrousseau-csharp-config
Version:        0.0.7
Release:        1%{?dist}
Summary:        Shareable C# / .NET configuration providing standardized Roslyn analyzer, EditorConfig, and OmniSharp rules adhering to modern 2026 standards

License:        Apache-2.0 OR MIT
URL:            https://github.com/sebastienrousseau/csharp-config
Source0:        %{name}-%{version}.tar.gz
BuildArch:      noarch
Requires:       nodejs >= 20

%description
@sebastienrousseau/csharp-config is a shareable configuration preset with no runtime
...
install -pm 0644 omnisharp.json %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
install -pm 0644 Directory.Build.props %{buildroot}%{sitelib}/%{npm_scope}/%{npm_bare}/
