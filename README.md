...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/csharp-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Strict Roslyn Analyzers & EditorConfig Rules](#strict-roslyn-analyzers-editorconfig-rules) — before and after rule comparison
...
Install `@sebastienrousseau/csharp-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/csharp-config

# pnpm
pnpm add -D @sebastienrousseau/csharp-config

# yarn
yarn add -D @sebastienrousseau/csharp-config

# bun
bun add -d @sebastienrousseau/csharp-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/csharp-config.git
cd csharp-config
make            # check + test
```
...
- **Target environment**: Optimized for C# 12 / 13 (.NET 8/9).
...
```bash
cp node_modules/@sebastienrousseau/csharp-config/omnisharp.json ./omnisharp.json
cp node_modules/@sebastienrousseau/csharp-config/Directory.Build.props ./Directory.Build.props
```
...
`@sebastienrousseau/csharp-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/csharp-config` solves this with three deliberate engineering choices:
...
## Strict Roslyn Analyzers & EditorConfig Rules

Enforces strict nullable reference types, modern C# 12/13 pattern matching, and Roslyn code quality rules.
...
### After (@sebastienrousseau/csharp-config Enforced)
...
When targeting legacy .NET Framework 2.0-3.5 runtimes incompatible with SDK-style projects and modern analyzers.
